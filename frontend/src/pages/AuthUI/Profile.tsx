...
import { apiFetch, authHeaders, API_BASE, apiGet } from "../../lib/api";
...
export default function ProfileDetails() {
  ...
  const [loadingFlaggedReviews, setLoadingFlaggedReviews] = useState(false);
  const [showFlaggedReviews, setShowFlaggedReviews] = useState(false);
  ...
  async function fetchFlaggedReviews() {
    setLoadingFlaggedReviews(true);
    try {
      const response = await apiGet<{ success: boolean; data: FlaggedReview[] }>("/reviews/flagged");
      setFlaggedReviews(response.data || []);
    } catch (err) {
      console.error("Failed to fetch flagged reviews:", err);
    } finally {
      setLoadingFlaggedReviews(false);
    }
  }

  // Fetch flagged reviews when showFlaggedReviews is toggled
  useEffect(() => {
    if (showFlaggedReviews && flaggedReviews.length === 0) {
      fetchFlaggedReviews();
    }
  }, [showFlaggedReviews]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-2xl shadow mt-8">
      ...
      {/* Flagged Reviews Section - Only show for shoppers */}
      {user?.role === "registered_shopper" && (
        <div className="mt-8 pt-8 border-t border-gray-200">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold text-gray-800">Flagged Reviews</h2>
            <button
            ...
            >
              {showFlaggedReviews ? "Hide" : "Show"} Flagged Reviews
            </button>
          </div>

          {showFlaggedReviews && (
            <div className="space-y-4">
              {loadingFlaggedReviews && <div className="text-center text-gray-500 py-4">Loading...</div>}

              {!loadingFlaggedReviews && flaggedReviews.length === 0 && (
                <div className="text-center text-gray-500 py-8 bg-gray-50 rounded-lg">
                  <p className="font-medium">No flagged reviews</p>
                  <p className="text-sm mt-1">You don't have any reviews that have been flagged by administrators</p>
                </div>
              )}

              {!loadingFlaggedReviews &&
                flaggedReviews.map((review) => (
                  <div key={review._id} className="bg-red-50 border border-red-200 rounded-lg p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mb-2">Shop: {review.shopName}</p>
                        <p className="text-sm text-gray-700">{review.description}</p>
                        {review.images && review.images.length > 0 && (
                          <div className="flex gap-2 mt-3">
                            {review.images.map((img, idx) => (
                              <img key={idx} src={img} alt={`Review ${idx + 1}`} className="h-16 w-16 object-cover rounded border" />
                            ))}
                          </div>
                        )}
                        <p className="text-xs text-gray-500 mt-3">
                          Created: {new Date(review.createdAt).toLocaleDateString()} | Last updated: {new Date(review.updatedAt).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                    <div className="mt-3 pt-3 border-t border-red-200">
                      <p className="text-sm text-red-800 font-medium">⚠️ This review has been flagged by administrators and is no longer visible to other users.</p>
                    </div>
                  </div>
                ))}
            </div>
