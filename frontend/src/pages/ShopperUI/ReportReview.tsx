...
interface Review {
  _id: string;
  item: string; // Changed from itemId to item
  shopper: string;
  description: string;
...
const ReportReview: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReview = async () => {
      ...
      setLoading(true);
      try {
        const review = await apiGet<Review>(`/reviews/${reviewId}`);
        setReview(review);

        if (review) {
          // Fetch item name
          const item = await apiGet<any>(`/items/${review.item}`);
  ...
  if (!review) {
    return <div className="flex items-center justify-center min-h-screen text-gray-500 text-lg">Review not found.</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="flex flex-col gap-5 items-center m-5 justify-center">
        <form onSubmit={handleSubmit} className="w-full max-w-2xl rounded-2xl bg-white shadow-lg p-8 sm:p-10 space-y-6">
          {/* --- Review Summary --- */}
          <div className="border-b border-gray-200 pb-6">
            ...
            {review.images && review.images.length > 0 && (
              <div className="mb-4">
                <img src={review.images[0]} alt="Review" className="w-full h-48 object-cover rounded-lg" />
                {review.images.length > 1 && (
                  <div className="grid grid-cols-3 gap-2 mt-2">
                    {review.images.slice(1, 4).map((img, idx) => (
                      <img key={idx} src={img} alt={`Review image ${idx + 2}`} className="w-full h-24 object-cover rounded-lg" />
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${review.availability ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-gray-500">Please select the primary reason for reporting this review</p>
          ...
          <div className="space-y-3">
            ...
            <p className="text-sm text-gray-500">Provide any additional context that will help our moderation team</p>
            <textarea
              ...
              maxLength={1000}
            />
            <p className="text-xs text-gray-400 text-right">{details.length}/1000 characters</p>
          </div>
          ...
          <p className="text-sm text-gray-500 text-center pt-2">Reports are reviewed by our moderation team within 24-48 hours</p>
        </form>
      </div>
    </div>
  );
