...
const ItemSearch: React.FC = () => {
  ...
  const fetchItems = async (q: string, page: number = 1) => {
    ...
    try {
      ...
      const res = await apiGet<SearchResponse>("/search/items?" + new URLSearchParams(params));
  ...
  return (
    <div className="pb-32">
      ...
      <div className="flex flex-col gap-5 items-center m-5">
        ...
        {/* Show "No items found" when there's no results (with or without query) */}
        {!loading && !error && !isFallbackMode && results.length === 0 && suggestedShops.length === 0 && (
          <p className="text-center text-gray-500">{query ? `No items found for "${query}"` : "No items available"}</p>
        )}
            ...
            >
              ...
              <div className="w-full">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${result.item.availability ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                    {result.item.availability ? "Available" : "Out of Stock"}
                  </span>
                </div>

                {result.item.description && <p className="text-gray-600 mb-3">{result.item.description}</p>}

                {result.item.price !== undefined && <p className="text-xl font-semibold text-green-600 mb-2">${result.item.price.toFixed(2)}</p>}

                {result.item.cdcVoucherAccepted && <span className="inline-block px-2 py-1 bg-purple-100 text-purple-800 text-xs rounded">CDC Voucher Accepted</span>}
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {result.verifiedByOwner && <BadgeCheck className="text-green-700" size={20} />}
                    </div>
            ...
            >
              ...
              <div className="w-full">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">May have availability</span>
                </div>
