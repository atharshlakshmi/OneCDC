...
const EditReview: React.FC = () => {
  ...
  // Passed from SeeReviews via navigate()
  const { reviewId, userId } = location.state || {};

  // Find existing review and its related item
  const existingReview = reviews.find((r) => r.id === reviewId);
  const item = items.find((i) => i.id === existingReview?.itemId);

  // --- State Hooks ---
  const [status, setStatus] = useState<string>(
    existingReview ? (existingReview.rating >= 4 ? "Available" : "Unavailable") : ""
  );
