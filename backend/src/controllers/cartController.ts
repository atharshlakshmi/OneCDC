...
export const addToCart = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const result = await cartService.addShopToCart(shopperId, shopId, itemTag);

  const message = result.alreadyInCart ? (result.wasUpdated ? "Shop tag updated in cart" : "Shop is already in your cart") : "Shop added to cart";
 ...
 */
export const generateRoute = asyncHandler(async (req: AuthRequest, res: Response) => {
  const shopperId = req.user!.id;
  ...
  // Generate route
  const route = await mapsService.generateMostEfficientRoute(origin, destinations, mode || TransportMode.WALKING);
